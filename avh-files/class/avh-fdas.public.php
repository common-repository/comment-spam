...
class AVH_FDAS_Public {
	...
	function actionHandleCronCleanNonce ()
	{
		...
		if ( $options['general']['cron_nonces_email'] ) {
			...
			$subject = sprintf( '[%s] Comment Spam - ' . __( 'Cron - Clean nonces', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
	...
	function actionHandleCronCleanIPCache ()
	{
		...
		if ( $options['general']['cron_ipcache_email'] ) {
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( '[%s] Comment Spam - ' . __( 'Cron - Clean IP cache', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
			...
			if ( empty( $commentdata['comment_type'] ) ) { // If it's a trackback or pingback this has a value
				...
				if ( $nonce != $_POST['_avh_first_defense_against_spam'] ) {
					if ( 1 == $this->core->options['general']['emailsecuritycheck'] ) {
						...
						$subject = sprintf( __( '[%s] Comment Spam - Comment security check failed', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
					...
					$m .= __( '<p>Protected by: Comment Spam</p>', 'avhfdas' );
	...
	function checkStopForumSpam ( $ip ) {
		...
		if ( isset( $spaminfo['Error'] ) ) {
			...
			if ( isset( $spaminfo['Error'] ) && $options['sfs']['error'] ) {
				...
				$to = get_site_option( 'admin_email' );
				$subject = sprintf( __( '[%s] Comment Spam - Error detected', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
	...
	function handleSpammer ( $ip, $info, $whichaction='main' )
	{
		...
		if ( $sfs_email || $php_email ) {
			...
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( __( '[%s] Comment Spam - Spammer detected [%s]', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $ip );
			$message = '';
	...
	function handleSpammerCache ( $info )
	{
		...
		if ( $cache_email ) {
			...
			// General part of the email
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( __( '[%s] Comment Spam - Spammer detected [%s]', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $info['ip'] );
			$message = '';
	...
	{
		$message .= "\r\n" . '--' . "\r\n";
		$message .= sprintf( __( 'Your blog is protected by Comment Spam v%s' ), $this->core->version ) . "\r\n";
